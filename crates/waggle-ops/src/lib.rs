...
pub const MINT: OperationSpec = OperationSpec {
    ...
    args: &[
        ...
        ArgSpec { name: "tree", required: false, doc: "For a DIRECTORY target: build an INDEXED directory tree — one content-addressed node per folder, each carrying a trigram index and a Bloom summary, snapshot-pinned. Thousands of files mint in one call. `read` a folder token for its table of contents, `read --file <name>` for one file, `search` to span the whole tree in one call, `coverage` for a per-file receipt. One revocation covers the whole tree." },
...
pub const READ: OperationSpec = OperationSpec {
    ...
    forward: &[
        EdgeSpec { to: "read", why: "continue the window, follow the outline deeper, or open a folder's file by name with --file" },
        EdgeSpec { to: "search", why: "grep the artifact — or, on a folder token, the whole tree in one call" },
