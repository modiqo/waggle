...
\paragraph{The arithmetic, honestly.} One 40\,KB report, five consumers,
five turns each. Under copy semantics the report is placed five times
(200\,KB) and re-sent on every turn ($\approx 1$\,MB re-read), discountable
only within one vendor and one cache TTL. Under name semantics, five
handoffs place 150 bytes total; each consumer pulls its projection once
($\approx$2\,KB) or interrogates surgically at ${\sim}1$\,KB per question;
re-turns re-send the 30-byte line, not the report. The ratio is large, but
the deeper point is in the other columns: under name semantics the author
\emph{knows} the report was resolved five times and read fourteen, the
correction reached the writer that resolved early, and the exchange
replays on any machine. Under copy semantics that knowledge is not
expensive---it is nonexistent.
