...
socket, a million-event fold in 334\,\textmu{}s) and an evaluation over ten
...
answers, and a 180-file tree---in which nine models answer the same question
under four turn-matched, \emph{paired} handoffs (1705 runs). A raw path trails the
token even where it can read the bytes---on a PDF it reaches 72\,\% with
\texttt{pdftotext} against the token's 100\,\% at a fifteenth of the bytes---and
collapses where structure must be \emph{traversed} rather than described (33\,\% on
the chain); against it the token wins 124 paired comparisons and loses 9. Against
\emph{pasting} the token neither wins nor loses: 95.3\,\% against 95.6\,\%, a
statistical tie---bought at \textbf{25$\times$ fewer ingested bytes} and
9$\times$ less money. Pasting was never inaccurate; it was expensive, and it
left no record. It does collapse once, and instructively: asked to \emph{count}
across a 180-file tree, a copy holding every byte answers 53\,\% where the token
answers 97\,\%---context was never the missing thing. Two results outlast the
arms. The receipt is an \emph{oracle}: runs whose coverage showed the required
regions consumed were 98\,\% correct, and those showing them unconsumed 28\,\%,
a seventy-point collapse visible to an orchestrator that never read the answer.
...
other arm can make. Of 1728 runs, 1705 are analysed; the rest are excluded for
unrecovered transport errors, and we say so rather than counting them as model
failures. Because every arm answers the same (artifact, model) pairs, the arms
are \emph{paired}, and we report the sign test on the runs where two arms
disagreed---the only runs that carry information about the difference between
them.

\paragraph{Parity with the paste, at a twenty-fifth of the context.} Across all
1705 runs, \emph{waggle} answers 95.3\,\% and \emph{copy} 95.6\,\%. Paired, they
are indistinguishable: 18 runs where waggle is right and copy wrong, 19 the other
way. \emph{waggle+gate} answers 96.7\,\% against copy's 95.6\,\%, and we decline
to call that a win: 19 discordant pairs to 14, $p=0.49$. \textbf{The honest claim
is parity, not victory}---and parity is bought at \textbf{2{,}627 bytes of
ingested context against pasting's 83{,}284}, a factor of 25, for 6.6$\times$
fewer tokens and 9$\times$ less money ({\$}4.67 against {\$}51.30 for the same
1705 answers). Pasting was never inaccurate. It was expensive, and it left no
record; the substrate's case is that you can stop paying for it without paying
for it in correctness.

\paragraph{Where the path collapses.} The \emph{reference} arm---a path and
ordinary file tools, which is what an agent is handed today---answers 68.0\,\%,
and against it the substrate's advantage is not a matter of interpretation:
\textbf{124 paired wins to 9} ($p<0.05$). The average conceals two distinct
failures. On \texttt{pdf} it reaches only \textbf{72\,\%} \emph{with}
\texttt{pdftotext} in hand---and pays 8{,}915 bytes to do it, because the tool
dumps one flat blob and the needle sits past the first window, so the agent
grep-and-guesses; the token, extracting the same PDF once at mint and serving a
lensed slice, answers \textbf{100\,\%} at \textbf{583 bytes} (\S\ref{sec:opaque}).
...
chain the path scores \textbf{33\,\%}, for a second and
more interesting reason: \tok{grep} cannot search for a section whose name it has
not read yet. Lexical search is superb at finding what you can already describe;
it has no answer at all for structure you must \emph{traverse}. We remain equally
clear about the other direction: on a small local text file a \tok{grep} is still
cheaper than a token, and the substrate's case was never that it beats \tok{grep}
at \tok{grep}'s own game.
...
\textbf{53\,\%} of the time. \emph{waggle+gate} answers \textbf{97\,\%}. The
...
arm's score. Of the 822 runs whose coverage receipt showed the author's required
regions \emph{consumed}, \textbf{98\,\% were correct}. Of the 29 whose receipt
showed them \emph{unconsumed}, \textbf{28\,\%} were---a collapse of seventy
...
it}, 95.3\,\% against 95.6\,\%, 18 paired wins to 19. We report the gated arm's
96.7\,\% and decline to claim it: 19 discordant pairs to 14 is $p=0.49$, and a
one-point lead that survives no test is not a result. What the substrate buys is
not correctness but \emph{cost and accountability}---a twenty-fifth of the
context, a ninth of the money, and a receipt---and it is worth saying that
plainly rather than dressing parity as a win. Nor does the token always cost
less: on a small local text file a \tok{grep} is cheaper, and the substrate's
case was never that it beats \tok{grep} at \tok{grep}'s own game.
...
\emph{down}, from 84\,\% to 78\,\%. The traces show why. A model reads six of
...
chain---the same gate is worth eight and five points respectively. An
